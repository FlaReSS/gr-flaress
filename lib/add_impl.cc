...
template <>
inline void volk_add<double>(double* out, const double* add, unsigned int num)
{
    volk_64f_x2_add_64f(out, out, add, num);
}
} // namespace


template <class T>
typename add<T>::sptr add<T>::make(size_t vlen)
{
    return gnuradio::make_block_sptr<add_impl<T>>(vlen);
}


template <>
add_impl<float>::add_impl(size_t vlen)
    : sync_block("add_ff",
...
template <>
add_impl<gr_complex>::add_impl(size_t vlen)
    : sync_block("add_cc",
...
template <>
add_impl<double>::add_impl(size_t vlen)
    : sync_block("add_dd",
                 io_signature::make(1, -1, sizeof(double) * vlen),
                 io_signature::make(1, 1, sizeof(double) * vlen)),
      d_vlen(vlen)
{
    set_alignment(std::max(1, int(volk_get_alignment() / sizeof(double))));
}

template <class T>
add_impl<T>::add_impl(size_t vlen)
    : sync_block("add",
